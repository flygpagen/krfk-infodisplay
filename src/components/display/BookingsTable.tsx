...
function getStatusBadge(status: Booking['status']) {
  switch (status) {
    case 'active':
      return <span className="text-sm lg:text-base px-3 py-1 rounded bg-green-500/20 text-green-400 animate-pulse">
          ● Pågående
        </span>;
    case 'upcoming':
...
export function BookingsTable() {
  ...
  const sortedBookings = useMemo(() => {
    return [...bookings].filter(b => b.status !== 'completed').sort((a, b) => {
  ...
  const remainingFlights = sortedBookings.filter(b => b.status === 'active' || b.status === 'upcoming').length;
  ...
  return <div className="flex flex-col h-full">
      <div className="flex items-center justify-between mb-3 lg:mb-5 min-h-[40px] lg:min-h-[52px]">
        ...
        <div className="flex items-center gap-4 lg:gap-6">
          ...
          {lastUpdate && <span className="text-sm lg:text-base text-muted-foreground">
      ...
      <div className="flex-1 overflow-auto rounded-lg border border-border/50">
        <Table>
          ...
          <TableBody>
            {sortedBookings.length === 0 ? <TableRow>
                <TableCell colSpan={4} className="text-center text-xl lg:text-2xl text-muted-foreground py-12 lg:py-16">
                  Inga bokningar idag
                </TableCell>
              </TableRow> : sortedBookings.map(booking => <TableRow key={booking.id} className={`${getStatusStyles(booking.status)} transition-colors`}>
