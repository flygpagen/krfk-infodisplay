...
export function SunTimesStrip() {
  ...
  const sunTimes = useMemo(() => {
    return getSunTimesForDate(new Date(), config.location.lat, config.location.lon);
  }, [config.location.lat, config.location.lon]);

  return (
    <div className="flex items-center justify-center px-4 lg:px-10 py-3 lg:py-4 border-t border-border/50 bg-gradient-to-r from-card to-background">
